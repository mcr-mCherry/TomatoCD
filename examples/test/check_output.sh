...
# considered passed.
#
# Authoritative reproducibility check: the file size and PDF magic bytes
# are not bit-identical on different systems (Cairo + webshot2 timestamps),
# so we compare the *first page object length* instead — a robust proxy.
set -euo pipefail
